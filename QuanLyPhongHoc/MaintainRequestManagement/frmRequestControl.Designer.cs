...
{
    partial class frmRequestControl
    {
        /// <summary>
        ...
        {
            System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(frmRequestControl));
            ...
            // 
            panel_PaginationPanel.Controls.Add(button1);
            ...
            panel_PaginationPanel.Size = new Size(1308, 40);
            panel_PaginationPanel.TabIndex = 0;
            // 
            // button1
            // 
            button1.Location = new Point(268, 6);
            ...
            panel_PaginationContainer.Location = new Point(1058, 0);
        ...
        private void ConfigureDataGridView()
        {
            ...
            dataGridView_Request.CellClick += DataGridView_Request_CellClick;
        }

        private void DataGridView_Request_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex < 0)
            {
                return;
            ...
            DataGridViewRow rowSelected = dataGridView_Request.SelectedRows[0];
            string ID = rowSelected.Cells[2].Value.ToString();

            if (e.ColumnIndex == dataGridView_Request.Columns["EditButton"].Index)
            {
                frmRequestAddOrUpdate = ActivatorUtilities.CreateInstance<frmRequestAddOrUpdate>(serviceProvider, int.Parse(ID));
            ...
            else if (e.ColumnIndex == dataGridView_Request.Columns["DeleteButton"].Index)
            {
                DialogResult result = MessageBox.Show("Bạn có chắc muốn xóa không?", "", MessageBoxButtons.YesNo, MessageBoxIcon.Question);

                if(result == DialogResult.Yes)
                {
                    MaintainRequest requestRemoved = unit.RequestManagement.Get(rq => rq.ID == int.Parse(ID));
                    unit.RequestManagement.Remove(requestRemoved);
                    unit.Save();
                    // Update lại datagridview
                    dataGridView_Request.Rows.RemoveAt(e.RowIndex);
                    frmCustomNotification notification = new frmCustomNotification("Đã xóa thành công", "success");
                    notification.Show();
                }
            }
        }
